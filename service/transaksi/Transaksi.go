...
func Read_Transaksi(Request request.Read_Transaksi_Request, Request_filter request.Read_Transaksi_Filter_Request) (response.Response, error) {
	...
	for i := 0; i < len(arr_data); i++ {
		...
		err = con.Table("barang_transaksi").Select("kode_barang_transaksi", "barang_transaksi.kode_inventory", "nama_barang", "barang_transaksi.jumlah_barang", "barang_transaksi.harga", "barang_transaksi.sub_total").Joins("join inventory s on s.kode_inventory = barang_transaksi.kode_inventory", "barang_transaksi.nama_satuan", "barang_transaksi.kode_barang_transaksi_inventory").Where("kode_transaksi = ?", arr_data[i].Kode_transaksi).Scan(&arr_data[i].Barang_transaksi)
